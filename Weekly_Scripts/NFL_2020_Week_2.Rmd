...
weeks = list(1,2) #Setting the weeks #add in additional weeks
inst.picks = list(week_1, week_2) #add in the additional weeks
cdt.picks = list(cadet_wk1, cadet_wk2) #add in the additional weeks
```

```{r read in scores clean data, include=FALSE}
Scores = read_csv("../CSV_Data_Files/NFL_Scores_2.csv") #%>% janitor::clean_names()
...
weekly_indiv_percent %>% 
  ...
  geom_line(aes(x = as.factor(Week), y = Percent, color = Name, 
                group = Name))+
  geom_text_repel(aes(label=Percent),hjust=0.5, vjust=1.5, size = 3)+
  ylim(c(0, 1)) +
  labs(x = "NFL Week", 
       y = "Correct Percentage", 
       title = "Weekly Individual Correct Percentage")+
...
c_weekly_indiv_percent %>% 
  ...
  geom_line(aes(x = as.factor(Week), y = Percent, color = Name, 
                group = Name))+
  geom_text_repel(aes(label=Percent),hjust=.5, vjust=1.5, size = 3)+
  #geom_text(aes(label=Percent),hjust=.5, vjust=-1.5)+
  ylim(c(0, 1)) +
  labs(x = "NFL Week",
       y = "Correct Percentage", 
